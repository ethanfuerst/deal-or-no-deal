...
class DealOrNoDeal:
    ...
    def print_cases(self):
        ...
        print(
            "   "
            + "  ".join([str(i) if i.available else "    " for i in self.cases[7:13]])
        )
        print("  ".join([str(i) if i.available else "    " for i in self.cases[13:20]]))
        print(
            "   "
            + "  ".join([str(i) if i.available else "    " for i in self.cases[20:]])
        )
        ...
        for i, j in zip(
            self.case_vals[: len(self.case_vals) // 2],
            self.case_vals[len(self.case_vals) // 2 :],
        ):
    ...
    def get_dealer_offer(self):
        ...
        return round(
            (sum(self.vals_left) / len(self.vals_left))
            * (random.randrange(75, 86) / 100),
            2,
        )
    ...
    def get_final_choice(self):
        ...
        self.print_cases()
        decision = input(
            "Do you want your original case (Y) or the last case left (N)? "
        )
